...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{deploymentView.png}
...
\Begin{
    $N\leftarrow New Fixed Appointment$\;
        ...
        {\uIf{$Flexible Appointments In (N_{end}-N_{start})=1$}{
            ...
            \uIf{$Flexible Appointment\ before\ N_{start} $ {\bf and} $Previous Fixed Appointment_{end}-N_{start} > Flexible Appointment_{minDuration} + TimeToGetToN$ {\bf and} $possibleInterval(Flexible Appointment, Previous Appointment_{end}, N_{start})$}{
            $relocate(Flexible Appointment)$\;
            $insert(N)$\;
            $return\ True$\;}
            
            \uElseIf{$Flexible Appointment\ after\ N_{start} $ {\bf and} $Next Fixed Appointment_{start}-N_{End} > Flexible Appointment_{minDuration} + TimeToGetToN$ {\bf and} $possibleInterval(Flexible Appointment, N_{end}, Next Appointment_{start})$}{
            $relocate(Flexible Appointment)$\;
            $insert(N)$\;
            $return\ True$\;}
        }
        \uElseIf{$Flexible Appointments In (N_{end}-N_{start})=2$}{
            ...
            \uIf{$Flexible Appointment1\ before\ N_{start} $ {\bf and} $Previous Fixed Appointment_{end}-N_{start} > Flexible Appointments_{minDuration} + TimeToGetToN$ {\bf and} $possibleInterval(Flexible Appointment1, Previous Fixed Appointment_{end}, N_{start})$ {\bf and} $Flexible Appointment2_{start}\ after\ N_{end} $ {\bf and} $Next Appointment_{start}-N_{end} > Flexible Appointment_{minDuration} + TimeToGetToN$ {\bf and} $possibleInterval(Flexible Appointment, N_{end}, Next Appointment_{start})$}{
...
\Begin{
    $N\leftarrow New Flexible Appointment$\;
        \uIf{$N_{timeInterval}=free$}{
            $insert(N, N_{start}, N_{start}+N_{minDuration})$\;
            $return True$\;}
        {\uIf{$Fixed Appointments In (N_{end}-N_{start})$}{
        
            \uIf{$AppointmentAfter_{start} - AppointmentBefore_{end} > N_{minDuration} + TimeToGetToN $}{
                $insert(N, AppointmentBefore_{end}, AppointmentBefore_{end}+N_{minDuration})$\;
                $return\ True$\;
            }
        }
        $return False$\;
...
\Begin{
    ...
    $T2\leftarrow Previous Appointment_{startTime}$\;
