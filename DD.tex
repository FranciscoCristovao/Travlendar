...
\SetAlgoLined
\Begin{
    $A\leftarrow Set of Appointments Surrounding N Time interval$\;
    $F\leftarrow Set of Flexible Appointments$\;
    $N\leftarrow New Appointment$\;
    \For{$i\leftarrow 1$ \KwTo $|A|$}{
        \uIf{$N_{timeInterval}=free$}{
            $insert(N)$\;
            $return$\;}
        {\uElseIf{$Flexible Appointments In N_{end}-N_{start}$}{
            \For{$j\leftarrow 1$ \KwTo $|F|$}{
            $rescheduleFlexibleAppointment(F[j])$
            }
            \If{$N_{timeInterval}=free$}{
            $insert(N)$\;
            $return$\;}
        }}
